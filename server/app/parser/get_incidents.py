...
def get_raw_data():
    ...
    try:
        ...
        data = responce.json()
        with open('parser/raw_incidents.json', 'w', encoding='utf-8') as file:
...
def prepare_data():
    with open('parser/raw_incidents.json', 'r', encoding='utf-8') as file:
    ...
    try:
        ...
        with open('parser/incidents.json', 'w', encoding='utf-8') as file:
...
def main():
    with open('parser/scheduler.txt', 'a', encoding='utf-8') as file:
        file.write(f'Запущено get_data🔄: {datetime.now().strftime("%d.%m.%Y %H:%M")}\n')
    print(get_raw_data())
    print(prepare_data())
    with open('parser/scheduler.txt', 'a', encoding='utf-8') as file:
        file.write(f'Завершено get_data✅: {datetime.now().strftime("%d.%m.%Y %H:%M")}\n\n')
